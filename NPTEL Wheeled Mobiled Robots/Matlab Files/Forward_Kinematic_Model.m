...
for i = 1:length(t)
    ...
    J_psi = [cos(psi),-sin(psi),0;
             sin(psi),cos(psi),0;
             0,0,1;];
    % inputs in forward kinematics
    u = 2; % x_axis velocity w.r.t B frame.
    v = 0; % y_axis velocity w.r.t B frame.
    r = 0.3; % angular velocity w.r.t B frame.
...
l = 0.8; %length of mobile robot
w = 0.6; %width of mobile robot
...
for i = 1:length(t) % animation starts here
    ...
    hold on, grid on; axis([-10 10 -2 20]), axis square
